...
import SubmittedModalContent, {
  SubmittedModalContentProps,
} from '@/components/modal/submittedModalContent';
        ...
        <div className='relative flex min-h-screen items-center  justify-center text-center'>
          ...
          >
            <Dialog.Overlay
              className={clsx(
                isDesktop
                  ? 'bg-[rgb(0,0,0,0.4)]  backdrop-blur-[10px]'
                  : ' bg-[rgb(0,0,0,0.8)]',
          ...
          >
            <div
              className={clsx(
                transparent ? '' : 'border border-secondary-200 bg-white',
                isDesktop ? MAX_WIDTH_CLASS_MAPPING[maxWidth] : '',
                isDesktop
                  ? `w-full`
                  : 'mx-auto max-h-[85vh] w-[85vw] overflow-y-auto',
                'm-10 inline-block transform overflow-hidden rounded p-4 text-left align-bottom'
              )}
            >
              {children}
            </div>
